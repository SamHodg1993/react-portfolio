...
export default function Footer() {
  ...
  const sendContactRequest = (title, body) => {
    axios.post('https://sam-hodgkinson.co.uk:8080/api/contact/send', {
  ...
  return (
    <div className="footer-container" id="footer">
      <Modal isOpen={contactModalOpen} onRequestClose={() => setContactModalOpen(false)} style={{
        content: {
          top: '50%',
          left: '50%',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: "100%", position: "relative" }} >
          <input placeholder="Enter a title" style={{
            marginBottom: "2rem",
          ...
          <div style={{ display: "flex", position: 'absolute', bottom: "10px" }}>
            <button style={{ height: "2rem", width: "10rem", backgroundColor: '#181818' }} onClick={() => { sendContactRequest(contactTitle, contactBody) }}>Send</button>
            <button style={{ height: "2rem", width: "10rem", backgroundColor: '#FF5555' }} onClick={() => setContactModalOpen(false)}>Cancel</button>
          </div>
        </div>
      </Modal >
